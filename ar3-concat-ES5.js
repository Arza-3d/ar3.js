...
--------------------------*/
{
  ...
  if ($navTarget.length !== 0) {
    var constructNote = 'ar3-aside-nav_done',
        isConstructed = $('body').hasClass(constructNote); // update 2 start
    ...
    for (var i = 0; i < $navTarget.length; i++) {
      // insert aside
      $($navTarget[i]).before('<aside id="r3-aside-nav-' + (i + 1) + '" class="r3-aside-nav">'); // create id and class for h1-h6
...

{
...

  {
    ...
    var text1 = isIndo ? 'tunjukkan semua kode' : 'show all code',
        text2 = isIndo ? 'persingkat kode' : 'short code'; // 2nd update end
...

{
  ...
  if (!_isConstructed3) {
    ...
    $('code, pre').attr('translate', 'no'); // 3.

    $('.comment-r3').attr('translate', 'yes'); // 4.

    $('video').attr('controls'); // 5.
    ...
    if ($youtube.length > 0) {
...

      {
        ...
        for (var _i3 = 0; _i3 < $youtube.length; _i3++) {
          ...
          youtubeLink = $($youtube[_i3]).attr('src'); // 6.c
    ...
    var $bpue = $("iframe[src*='blueprintue']");

    if ($bpue.length > 0) {
      $bpue.attr('scrolling', 'no');

      for (var _i4 = 0; _i4 < $bpue.length; _i4++) {
        var bpueTag = $bpue[_i4].outerHTML;

        var _endCharIndex = bpueTag.indexOf('></iframe>');

        $bpue[_i4].outerHTML = bpueTag.slice(0, _endCharIndex) + ' allowFullScreen></iframe>';
      }
    }
  }
...

{
  ...
  if (!_isConstructed4) {
    // 1.
    $('table, pre').wrap('<div style="overflow:auto">'); // 2.

    {
      var $img = $('img').not('code > img, h3 > img, img[src$=".svg"],  img[src$=".png"]');
    ...
    $('main section + h3, aside > nav + .r3-accordion-B, aside > a + .r3-accordion-B').before('<hr>');
    {
      ...
      if ($h4s != null) {
        for (var _i6 = 0; _i6 < $h4s.length; _i6++) {
          var $h4 = $($h4s[_i6]);
