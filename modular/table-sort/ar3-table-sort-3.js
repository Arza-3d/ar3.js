...
(function() {
    ...
    function switchTablesRow() {
      ...
      for (h = 0; h < tables.length; h++) {
        ...
        let columnSort = 1;
        if ($(tables[h]).attr('data-column-sort-r3') !== undefined) {
          columnSort = parseInt($(tables[h]).attr('data-column-sort-r3'));
        }
        function switchTableRow() {
          while (switching) {
              ...
              for (i = 1; i < (rows.length - 1); i++) {
                  shouldSwitch = false;
                  x0 = rows[i].getElementsByTagName("TD")[columnSort - 1].innerHTML.toLowerCase();
                  x = getComment(x0);

                  y0 = rows[i + 1].getElementsByTagName("TD")[columnSort - 1].innerHTML.toLowerCase();
                  y = getComment(y0);
        ...
        switchTableRow();
          ...
          row1test = tables[h].querySelector("tbody > tr:nth-child(1) > td:nth-child("+ columnSort +")").innerHTML.toLowerCase();
          row2test = tables[h].querySelector("tbody > tr:nth-child(2) > td:nth-child("+ columnSort +")").innerHTML.toLowerCase();
